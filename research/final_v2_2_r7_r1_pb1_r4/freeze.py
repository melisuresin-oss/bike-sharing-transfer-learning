"""Append-only PB1 R4 final freeze with transitive verified-snapshot binding."""
...
def freeze(authorization_path: Path, stage_root: Path, raw_data_root: Path,
    ...
    final_paths = (output_root / "final_validation_report.json", output_root / "frozen_result_payload_manifest.json",
                   output_root / "final_result_archive.zip", output_root / "final_result_manifest.json")
    if any(path.exists() for path in final_paths):
        raise FileExistsError("repeat/conflicting final freeze publication")
    report = validator.validate_results(authorization_path, stage_root, raw_data_root,
    ...
    names = ["authorization.json", "first_label_access.json", "materialized_targets.npz",
             "target_materialization_provenance.json", "metrics.json", "bootstrap_summary.json",
             "bootstrap_replicates.npz", "paired_comparisons.json", "final_validation_report.json"]
    files = [core.entry(core.no_symlink_path(output_root / name), relative_to=stage_root) for name in names]
    target_authority_entry = core.entry(core.no_symlink_path(target_authority_path), relative_to=stage_root)
    files.append(target_authority_entry)
    payload_manifest = {
        "schema_version": core.PAYLOAD_MANIFEST_SCHEMA, "status": "PB1_R4_RESULT_PAYLOAD_FROZEN",
        ...
        "pb1_package_manifest_sha256": pb1_package_sha,
    ...
    bound = {
        ...
        "pb1_package_manifest": core.entry(core.no_symlink_path(pb1_package_manifest), relative_to=stage_root),
    }
    final_manifest = {
        "schema_version": core.FINAL_MANIFEST_SCHEMA, "status": "FINAL_V2_2_RESULTS_FROZEN",
        "bound_artifacts": bound, "verified_snapshot_root_sha256": snapshot_root,
        ...
        "training_performed_by_pb1": False, "predictions_modified_by_pb1": False,
        "new_learned_checkpoints": 0,
    }
    manifest_entry = core.atomic_json(final_paths[3], final_manifest)
    return {"status": "FINAL_V2_2_RESULTS_FROZEN", "final_result_manifest": manifest_entry,
            "final_result_archive": archive_entry, "verified_snapshot_root_sha256": snapshot_root}
