...
MANIFEST='deployment/final_v2_2_a40/BUNDLE_MANIFEST_DATA_GATE_R4.json'
def check(sha):
 ...
 v=verify_data_gate.verify();return {'status':'PASS','package_members':len(m['files']),'neural_fits':v['neural_fits'],'evaluation_passes':v['evaluation_passes'],'reporting_cells':v['reporting_cells'],'phase_a_execution_data_constructed':True,'phase_b_targets_materialized':False,'final_predictions_generated':False,'final_metrics_computed':False,'final_experiment_started':False,'optimizer_updates':0,'model_forward_calls_on_final_labels':0}
def main():
